...
export default function MailTemplates() {
  const [templates, setTemplates] = useState<MailTemplate[]>(mockMailTemplates)
  const [selectedType, setSelectedType] = useState<MailTemplateType | 'all'>('all')
  ...
  const handleSave = () => {
    ...
    if (isAdding) {
      ...
      const newTemplate: MailTemplate = {
        ...
        variables: mailTemplateTypes[formData.type || 'custom'].defaultVariables,
  ...
  const getAvailableVariables = (type: MailTemplateType) => {
    return mailTemplateTypes[type]?.defaultVariables || []
  }
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          <h1 className="text-2xl font-bold">Pochta Xat Shablonlari</h1>
          <p className="text-sm text-muted-foreground mt-1">
            Xat shablonlarini boshqarish va tahrirlash
          </p>
        </div>
      ...
      <Card className="banking-card">
        <CardContent className="pt-6">
          <div className="flex flex-wrap gap-2">
            ...
            {(Object.keys(mailTemplateTypes) as MailTemplateType[]).map((type) => {
              ...
              return (
                ...
                >
                  {mailTemplateTypes[type].label} ({count})
                </button>
              )
            })}
          </div>
      ...
      {(isAdding || editingId) && (
        <Card className="banking-card border-primary/20">
          <CardHeader>
            <CardTitle>{isAdding ? 'Yangi xat shablon qo\'shish' : 'Xat shablonni tahrirlash'}</CardTitle>
          </CardHeader>
          <CardContent className="space-y-4">
            ...
            <div className="space-y-2">
              ...
              <select
                ...
                onChange={(e) => {
                  const newType = e.target.value as MailTemplateType
                  setFormData({ 
                    ...formData, 
                    type: newType,
                    content: formData.content || ''
              ...
              >
                {(Object.keys(mailTemplateTypes) as MailTemplateType[]).map((type) => (
                  <option key={type} value={type}>
                    {mailTemplateTypes[type].label}
                  </option>
                ))}
              </select>
              <p className="text-xs text-muted-foreground">
                {mailTemplateTypes[formData.type || 'custom'].description}
              </p>
            </div>
            ...
            <div className="space-y-2">
              ...
              <textarea
                ...
                placeholder="Xat shablon matnini kiriting..."
                rows={8}
                className="w-full px-3 py-2 rounded-lg text-sm border border-border bg-background focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring transition-colors resize-none"
              />
                    ...
                    <div className="flex items-center gap-3 mb-3">
                      ...
                      <div className="flex items-center gap-2">
                        ...
                        <span className="px-2.5 py-1 text-xs font-medium bg-primary/10 text-primary rounded">
                          {mailTemplateTypes[template.type].label}
                        </span>
                      </div>
                    </div>
                  </div>
