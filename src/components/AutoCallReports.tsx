...
const getCurrentYearAndMonth = () => {
  ...
  const hasCurrentMonth = mockAutoCallReportsData.some(
  ...
  const latestData = mockAutoCallReportsData.sort((a, b) => {
...
export default function AutoCallReports() {
  ...
  const availableYears = useMemo(() => {
    const years = new Set(mockAutoCallReportsData.map((item) => item.year))
  ...
  const availableMonths = useMemo(() => {
    const months = new Set(
      mockAutoCallReportsData.filter((item) => item.year === selectedYear).map((item) => item.month)
    )
  ...
  const filteredData = useMemo(() => {
    let data = mockAutoCallReportsData.filter((item) => item.year === selectedYear)
  ...
  const totals = useMemo(() => getAutoCallTotals(filteredData), [filteredData])
  ...
  const madeRate = calculatePercentage(totals.made, totals.needed)
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          <h1 className="text-2xl font-bold">Avtozvon Hisobotlari</h1>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        <Card className="banking-card">
          <CardHeader>
            <div className="flex items-center justify-between">
              <CardTitle className="text-base">Jami qilish kerak bo'lgan qo'ng'iroqlar</CardTitle>
              <Phone className="w-5 h-5 text-primary" />
            </div>
          </CardHeader>
          <CardContent>
            <div className="text-3xl font-bold mb-2">{formatNumber(totals.needed)}</div>
            <div className="text-sm text-muted-foreground">Jami kerakli qo'ng'iroqlar</div>
          </CardContent>
        </Card>
        ...
        <Card className="banking-card">
          <CardHeader>
            <div className="flex items-center justify-between">
              <CardTitle className="text-base">Qilingan</CardTitle>
              <TrendingUp className="w-5 h-5 text-blue-600" />
            </div>
          </CardHeader>
          <CardContent>
            <div className="text-3xl font-bold mb-2 text-blue-600">{formatNumber(totals.made)}</div>
            <div className="text-sm text-muted-foreground mb-2">
              Shulardan istemolchiga qilingan qo'ng'iroqlar
            </div>
            <div className="flex items-center justify-between">
              <span className="text-sm text-muted-foreground">Qilish foizi</span>
              <span className="text-sm font-semibold">{madeRate}%</span>
            </div>
            <div className="w-full bg-muted rounded-full h-2 mt-2">
              <div
                className="bg-blue-500 h-2 rounded-full transition-all"
                style={{ width: `${madeRate}%` }}
              />
            </div>
          </CardContent>
        </Card>
                    ...
                    <div key={`${item.month}-${item.year}-${index}`} className="space-y-2">
                      ...
                      <div className="relative w-full h-10 bg-muted rounded-lg overflow-hidden">
                        ...
                        <div
                          ...
                          style={{ width: `${itemMadeRate}%` }}
                        >
                          {itemMadeRate > 15 && (
                            <span className="text-xs font-medium text-white">
                              {formatNumber(item.autoCall.made)}
                            </span>
                          )}
                        </div>
                        {item.autoCall.notMade > 0 && (
                          <div
                            className="absolute right-0 top-0 h-full bg-warning-500/80 flex items-center justify-start pl-2 transition-all"
                            style={{ width: `${calculatePercentage(item.autoCall.notMade, item.autoCall.needed)}%` }}
                          >
                            {calculatePercentage(item.autoCall.notMade, item.autoCall.needed) > 15 && (
                              <span className="text-xs font-medium text-white">
                                {formatNumber(item.autoCall.notMade)}
                              </span>
                            )}
                          </div>
                        )}
            ...
            <div className="space-y-4">
              <div className="grid grid-cols-2 gap-4">
                <div className="space-y-1">
                  <div className="text-sm text-muted-foreground">Jami qilish kerak bo'lgan qo'ng'iroqlar</div>
                  <div className="text-lg font-semibold">{formatNumber(totals.needed)}</div>
                </div>
                <div className="space-y-1">
                  <div className="text-sm text-muted-foreground">Qilingan</div>
                  <div className="text-lg font-semibold text-blue-600">{formatNumber(totals.made)}</div>
                </div>
                <div className="space-y-1">
                  <div className="text-sm text-muted-foreground">Qilinmagan</div>
                ...
                <div className="space-y-1">
                  <div className="text-sm text-muted-foreground">Qilish foizi</div>
                  <div className="text-lg font-semibold">{madeRate}%</div>
                </div>
              </div>
            </div>
          ) : (
              ...
              <table className="w-full border-collapse">
                ...
                <tbody className="divide-y divide-border">
                    ...
                    return (
                      ...
                      >
                        ...
                        <td className="px-4 py-3 text-sm text-right font-semibold">
                          {formatNumber(item.autoCall.needed)}
                        </td>
                  ...
                  <tr className="bg-primary/5 font-semibold border-t-2 border-primary">
                    ...
                    <td className="px-4 py-3 text-sm text-right text-blue-600">{formatNumber(totals.made)}</td>
                    <td className="px-4 py-3 text-sm text-right text-warning-700">{formatNumber(totals.notMade)}</td>
                    ...
                    <td className="px-4 py-3 text-sm text-right">{madeRate}%</td>
                  </tr>
                </tbody>
              </table>
            </div>
