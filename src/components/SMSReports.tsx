...
const getCurrentYearAndMonth = () => {
  ...
  const hasCurrentMonth = mockReportsData.some(
  ...
  const latestData = mockReportsData.sort((a, b) => {
...
export default function SMSReports() {
  ...
  const availableYears = useMemo(() => {
    const years = new Set(mockReportsData.map((item) => item.year))
  ...
  const availableMonths = useMemo(() => {
    const months = new Set(
      mockReportsData.filter((item) => item.year === selectedYear).map((item) => item.month)
    )
  ...
  const filteredData = useMemo(() => {
    let data = mockReportsData.filter((item) => item.year === selectedYear)
  ...
  const totals = useMemo(() => getTotals(filteredData), [filteredData])
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          <h1 className="text-2xl font-bold">SMS Hisobotlari</h1>
        ...
        <Card className="banking-card">
          <CardHeader>
            <div className="flex items-center justify-between">
              <CardTitle className="text-base">Jami yuborilishi kerak bo'lgan SMS</CardTitle>
              <MessageSquare className="w-5 h-5 text-primary" />
            </div>
          </CardHeader>
          <CardContent>
            <div className="text-3xl font-bold mb-2">{formatNumber(totals.needed)}</div>
            <div className="text-sm text-muted-foreground">Jami kerakli SMS</div>
          </CardContent>
        </Card>
            ...
            {isSingleMonth ? (
              ...
              <div className="space-y-3">
                {filteredData.map((item, index) => {
                  const itemSendRate = calculatePercentage(item.sms.sent, item.sms.needed)
                  return (
                      ...
                      <div className="relative w-full h-10 bg-muted rounded-lg overflow-hidden">
                        ...
                        {item.sms.notSent > 0 && (
                          <div
                            ...
                            style={{ width: `${calculatePercentage(item.sms.notSent, item.sms.needed)}%` }}
                          >
                            {calculatePercentage(item.sms.notSent, item.sms.needed) > 15 && (
                              <span className="text-xs font-medium text-white">
                                {formatNumber(item.sms.notSent)}
                              </span>
                            )}
                          </div>
                        )}
            ...
            <div className="space-y-4">
              <div className="grid grid-cols-2 gap-4">
                <div className="space-y-1">
                  <div className="text-sm text-muted-foreground">Jami yuborilishi kerak bo'lgan SMS</div>
